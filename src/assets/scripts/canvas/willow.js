...
class Plant {
  ...
  init () {
    for (let i = 0; i < this.resolution; i++) {
      ...
      const y = this.position.y + (0, i * this.div)
  ...
  update () {
    for (let i = 0; i < this.points.length; i++) {
      ...
      const velocity = point.position.copy()
    ...
    for (let i = 0; i < 4; i++) {
      this.links.forEach(link => {
        const distance = link.p0.position.dist(link.p1.position)
        const difference = link.length - distance
...
let plants = []
const gravity = new Vector(0, 0.6)
const wind = {
  ...
  update: function () {
    const time = new Date() - this.time_start
...
const backgroundColor = '#1d0d21'

function populate () {
  plants = []
  const div_x = 4
  const div_y = 5
  const total_div = div_x * div_y

  const part_x = W / div_x

  for (let i = 0; i < total_div; i++) {
    // offset_x = (i % div_x) * part_x,
    const offset_x = Util.random(0, W)
    const offset_y = Math.floor(i / div_x)
    let z_value = Util.map(offset_y, 0, div_y - 1, 0.8, 0)
    const p_length = Util.map(offset_y, 0, div_y - 1, H * 0.4, H)

    const z_random = Util.random(0.8, 1.2)
    z_value *= z_random
    z_value = Util.clamp(z_value, 0, 1)
    const plant = new Plant(offset_x, 0, p_length)
    plant.setZ(z_value)
    plant.init()
    plants.push(plant)
  }
